...
app.define('sync', params('token:string'), function(ctx, next){
  ...
  function syncChunk(cb){
    self._syncChunk(ctx.user.id, ctx.noteStore, function(err, usn){
...
app.define('_syncChunk', params('id', 'noteStore'), function(ctx, next, end){
  ...
  ctx.noteStore.getSyncChunk(lastUpdated, 20, !lastUpdated, function(err, chunk){
    if(err) return next(err);
    ctx.chunk = chunk;
    next();
  });
  ...
  (ctx.chunk.notebooks || []).forEach(function(doc){
    ...
    self.roda.store.put(doc.guid, doc, ctx.transaction);
  });
  ...
  (ctx.chunk.notes || []).forEach(function(doc){
    ctx.transaction.defer(function(cb){
      ...
      ctx.noteStore.getNoteContent(doc.guid, function(err, content){
        ...
        doc.userId = ctx.meta._id;
        doc.content = content;
        self.roda.store.put(doc.guid, doc, ctx.transaction, cb);
      });
    });
  });
  ...
  [].concat(
    ...
    self.roda.store.del(guid, ctx.transaction);
  });
