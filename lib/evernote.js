...
app.define('_syncChunk', params('id', 'noteStore', 'full:boolean'), function(ctx, next, end){
  ...
  (ctx.chunk.notes || []).forEach(function(doc){
    ...
    if(self.resources && Array.isArray(doc.resources)){
      doc.resources.forEach(function(res){
        var exists = false;
        ctx.transaction.defer(function(cb){
          //resource key: user id + md5 hash
          //todo: full data loaded into memory. Not good.
          var key = doc.userId + '/' + Buffer(res.data.bodyHash).toString('hex');
          self.resources.exists({ key: key }, function(err, exists){
            if(exists) return cb(err);
            ctx.noteStore.getResource(res.guid, true, false, false, false, function(err, res){
              if(err) return cb(err);
              var stream = self.resources.createWriteStream({ key: key });
              stream.write(Buffer(res.data.body));
              stream.end(cb);
            });
          });
        });
...
app.define('_update', params('id', 'noteStore', 'doc'), function(ctx, next){
  ...
  switch(ctx.doc.type){
    ...
    case 'note':
      ...
      var resDirty = false;
      var resources = (ctx.doc.resources || []).map(function(res){
        var resource = new SDK.Resource();
        var attr = new SDK.ResourceAttributes();

        resource.guid = res.guid;
        if(!res.guid) resDirty = true;

        // extend(attr, res.attributes);
        // resource.attributes = attr;

        return resource;
      });

      if(resDirty) note.resources = resources;
